...
export const getSuggestedUsers = async (req, res) => {
    try {
        ...
        for (let u of users) {
            const userIdStr = u._id.toString();
            if (currentFriends.includes(userIdStr))
                continue;
            const fr = requestMap[userIdStr];
            if (fr) {
                if (fr.status === "accepted" || fr.status === "pending") {
...
export const acceptFriendRequest = async (req, res) => {
    try {
        ...
        if (!fromUserData || !toUserData)
            return res.status(404).json({ message: "User not found." });
        if (fromUserData.isDeleted || toUserData.isDeleted)
            return res.status(400).json({ message: "Friend request is no longer valid." });
...
export const cancelFriendRequest = async (req, res) => {
    try {
        ...
        await createNotificationInternal(request.from, request.to, NotificationType.FRIEND_CANCEL, undefined, `${fromUser?.fullName} cancelled the friend request`);
...
export const getFromAnToPendingRequests = async (req, res) => {
    try {
        const { userId } = req.params;
        if (!userId)
            return res.status(400).json({ message: "userId is required." });
...
export const getFriendUsers = async (req, res) => {
    try {
        ...
        const validRequests = friendRequests.filter((fr) => fr.from && fr.to);
        const friends = validRequests.map((fr) => {
...
export const getMutualFriends = async (req, res) => {
    try {
        ...
        const mutualIds = otherFriends.filter((f) => currentFriends.includes(f));
