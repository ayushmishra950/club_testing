...
export const replyToSuggestion = async (req, res) => {
    try {
        ...
        const suggestion = await Suggestion.findByIdAndUpdate(id, updateQuery, { new: true }).populate("createdBy", "fullName email profileImage");
        if (!suggestion) {
            return res.status(404).json({ message: "Suggestion not found." });
        }
...
export const markAsReadByAdmin = async (req, res) => {
    try {
        const { id } = req.params;
        const suggestion = await Suggestion.findByIdAndUpdate(id, { $set: { adminUnreadCount: 0 } }, { new: true }).populate("createdBy", "fullName email profileImage");
        if (!suggestion) {
            return res.status(404).json({ message: "Suggestion not found." });
        }
        const io = getIO();
