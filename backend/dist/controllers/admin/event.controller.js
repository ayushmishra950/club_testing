...
export const addEvent = async (req, res) => {
    try {
        ...
        if (imageUrls.length === 0)
            return res.status(400).json({ message: "coverImage upload failed." });
...
export const updateEvent = async (req, res) => {
    try {
        ...
        if (coverImage) {
            ...
            else if (Array.isArray(coverImage)) {
                oldImages = coverImage;
            }
        }
        let uploadedImages = [];
...
export const addAndRemoveCandidateFromEvent = async (req, res) => {
    try {
        const { eventId, userId } = req.body;
        if (!userId || !eventId)
            return res.status(400).json({ message: "eventId or  userId not Found." });
        ...
        if (event.interestedCandidate.some(id => id.equals(userObjectId))) {
            ...
            event.interestedCandidate = event.interestedCandidate.filter(id => !id.equals(userObjectId));
            // y normal format hai  
            await event.save();
