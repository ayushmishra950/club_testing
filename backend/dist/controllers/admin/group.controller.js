...
export const getAllGroupsByAdmin = async (req, res) => {
    try {
...
export const addMember = async (req, res) => {
    try {
        ...
        const existingMembers = group.members.map(m => m.toString());
        const newMembers = members.filter((id) => !existingMembers.includes(id));
        if (newMembers.length === 0) {
            return res.status(400).json({ message: "All users already in group" });
        }
        const chat = await Chat.findOneAndUpdate({ groupId }, {
...
export const removeMember = async (req, res) => {
    try {
        ...
        await group.save();
        // Update chat as well
        await Chat.findOneAndUpdate({ groupId }, {
            $pull: { members: userId }
        }, { upsert: true });
        const updateGroup = await group.populate([
            { path: "createdBy", select: "fullName email profileImage" },
