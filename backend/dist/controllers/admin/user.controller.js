...
export const handleVerifyUser = async (req, res) => {
    try {
        ...
        await User.updateMany({ _id: { $in: memberIds } }, { $set: { isVerified: true } });
...
export const handleVerifyBusinessUser = async (req, res) => {
    try {
...
export const handleBlockAndUnBlockUser = async (req, res) => {
    try {
...
export const roleAssignUser = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(userId, { role: role.toLowerCase() }, { new: true });
        if (!user) {
            return res.status(404).json({ message: "User not found." });
        }
        res.status(200).json({
...
export const activeAndInactiveUser = async (req, res) => {
    try {
        const userId = req.params.id;
...
export const updateUserByAdmin = async (req, res) => {
    try {
        ...
        if (allowedUpdates.children) {
            ...
            allowedUpdates.children = allowedUpdates.children.map((child) => ({
                name: child.name || "",
                age: child.age || 0
            }));
        }
        ...
        if (allowedUpdates.email || (originalUser.spouseEmail && originalUser.spouseName)) {
            ...
            if (allowedUpdates.email && allowedUpdates.email !== originalUser.email) {
                ...
                await User.updateMany({ spouseEmail: originalUser.email }, {
                    $set: {
            ...
            if (originalUser.spouseEmail) {
                ...
                if (spouseUser) {
                    ...
                    await User.findByIdAndUpdate(spouseUser._id, {
                        $set: {
                            spouseName: allowedUpdates.fullName || originalUser.fullName,
...
export const addBusinessUser = async (req, res) => {
    try {
        const io = getIO();
        ...
        const updatedBusinesses = await Promise.all(parsedBusinesses.map(async (biz, index) => {
            let imageUrl = biz.businessCoverImage || "";
            // Check for file in req.files
            const file = files?.find((f) => f.fieldname === `businessCoverImage_${index}`);
            if (file && file.buffer) {
...
export const adminConvertPremiumUser = async (req, res) => {
    try {
        const io = getIO();
        const { userId, amount, transactionNumber } = req.body;
        const file = req.files?.screenshot?.[0];
        if (!userId || !amount || !file)
            return res.status(400).json({ message: "userId, amount or screenshot is required." });
        const user = await User.findById(userId);
        if (!user)
            return res.status(404).json({ message: "user not found." });
...
export const uploadExcel = async (req, res) => {
    try {
        ...
        const existingEmailSet = new Set(existingUsers.map(u => normalize(u.email)).filter(Boolean));
        const existingPhoneSet = new Set(existingUsers.map(u => u.mobile?.toString()).filter(Boolean));
        ...
        for (const row of rows) {
            ...
            const isUserDuplicate = (userEmail && (existingEmailSet.has(userEmail) ||
                usedEmails.has(userEmail))) ||
                (userPhone && (existingPhoneSet.has(userPhone) ||
                    usedPhones.has(userPhone)));
            if (!isUserDuplicate) {
                if (userEmail)
                    usedEmails.add(userEmail);
                if (userPhone)
                    usedPhones.add(userPhone);
                successUsers.push({
            ...
            if ((spouseEmail || spousePhone) && row.spouseName) {
                const isSpouseDuplicate = (spouseEmail && (existingEmailSet.has(spouseEmail) ||
                    usedEmails.has(spouseEmail))) ||
                    (spousePhone && (existingPhoneSet.has(spousePhone) ||
                        usedPhones.has(spousePhone)));
                if (!isSpouseDuplicate) {
                    if (spouseEmail)
                        usedEmails.add(spouseEmail);
                    if (spousePhone)
                        usedPhones.add(spousePhone);
                    successUsers.push({
